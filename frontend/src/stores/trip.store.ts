...
export const useTripStore = create<TripState>((set) => ({
  ...
  createTrip: async (tripData) => {
    set({ loading: true });
    try {
      const res = await axiosInstance.post<Trip>("/api/trips", tripData);
  ...
  startTrip: async (tripId) => {
    ...
    try {
      await axiosInstance.put(`/api/trips/${tripId}/start`);
    } catch (error: any) {
  ...
  endTrip: async (tripId) => {
    set({ loading: true }); // Set loading to true
    try {
      await axiosInstance.put(`/api/trips/${tripId}/end`);
    } catch (error: any) {
      toast.error(error.response?.data?.message || "An error occurred"); // Display error message
    } finally {
      set({ loading: false }); // Set loading to false
  ...
  pickStudent: async (tripId, studentId) => {
    set({ loading: true }); // Set loading to true
    try {
      await axiosInstance.put(`/api/trips/${tripId}/pick/${studentId}`); // Make request to pick student
    } catch (error: any) {
      toast.error(error.response?.data?.message || "An error occurred"); // Display error message
    } finally {
      set({ loading: false }); // Set loading to false
  ...
  getTripById: async (id) => {
    set({ loading: true });
    try {
      const res = await axiosInstance.get<Trip>(`/api/trips/${id}`);
      set({ trips: [res.data] });
    } catch (error: any) {
      toast.error(error.response?.data?.message || "An error occurred");
    } finally {
